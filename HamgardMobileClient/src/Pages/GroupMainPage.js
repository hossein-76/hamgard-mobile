...
class GroupMainScreen extends React.Component {
  static navigationOptions = ({ navigation }) => {
    return {
      headerRight: (
        <View>
          <TextFa style={HeaderStyles.TitleRight}>صفحه‌ی گروه</TextFa>
        </View>
      ),
      headerStyle: {
  ...
  membersRenderItem = List => {
    return (
      <View style={{ margin: 5 }}>
        <GroupMember name={List.item.username} />
      </View>
    );
  };
    ...
    return (
      <View style={{ margin: 5 }}>
        <PollListItem
          ...
          voteCount={List.item.vote_count}
          id={List.item.id}
          onPress={() => {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.memberListContainer}>
          ...
          >
            <TextFa style={{ fontSize: 20 }}>
              تعداد : {this.props.group.members.length}
            </TextFa>
            <View
              style={{
                flexDirection: "row",
